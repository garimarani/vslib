...
@Controller
@RequestMapping("/setup/parameters/storage")
public class StorageParamsController extends SetupBaseController {
	...
	public String storageParameters(Model model, RedirectAttributes reat, Principal principal){
		...
		return "setup/parameters/storage";
	}
	...
	public String storageParameters(@ModelAttribute VslibParams vslibParams, BindingResult result, Model model, RedirectAttributes reat, Principal principal){
		...
		obj.setPhotosFolder(vslibParams.getPhotosFolder());
		obj.setEdocsFolder(vslibParams.getEdocsFolder());
		obj.setWorksFolder(vslibParams.getWorksFolder());
		
		obj.setAddDefaults(principal.getName());
		
		if (this.vslibService.addVslibParams(obj)){
			reat.addFlashAttribute("message", "Storage parameters updated successfully.");
			return "redirect:/setup/parameters";
		} else {
			reat.addFlashAttribute("message", "Storage parameters not updated.");
